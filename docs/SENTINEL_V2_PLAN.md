...
| P0-2 | Yükseltilmiş (UAC) kurulumların yolu okunamıyor | `SetupSentinelService.cs:214` (`proc.MainModule`) | Bakım `asInvoker` çalışıyor (`app.manifest:14`). Yönetici olarak çalışan `setup.exe`'nin modülüne erişim reddedilir. Bu durumda yalnızca adı tam olarak `setup`/`installer`/`msiexec`/`kurulum` olanlar yakalanır. Kurulumların çoğu UAC istediği için ya kaçıyor ya da `InstallerPath` boş kalıyor. |
