...
| S-17 | TrustedInstaller sessizce yöneticiye düşüyor | `SystemToolsService.cs:163-200` | DEN G-6 | Tamam (TrustedInstallerLaunchOutcome; dürüst fallback) |
...
| Faz 1 | Tamam (PC testi bekliyor) | `claude/charming-feynman-g2crt8` | v3.21.0. Core testleri: 218 (Linux) |
...
| Faz 7 | Tamam (PC testi bekliyor) | `claude/charming-feynman-g2crt8` | Nöbetçi v2: tur başına tek süreç görüntüsü + aday penceresi (eskiden her süreç için ayrı Toolhelp görüntüsü), kurulum çatısı parmak izi (Inno/NSIS/WiX/InstallShield/Advanced/7z/Squirrel/MSI) ve MOTW kaynağıyla puanlama; kurulum dosyası SHA-256 + imza + indirildiği site; sistem sensörü (IFEO, Winlogon, AppInit, PATH, proxy, tarayıcı politikaları, Defender istisnası, fiziksel kök sertifika depoları, hosts, güvenlik duvarı, görevler, sağ tık uzantıları); gürültü kuralları; kural tabanlı risk motoru (Temiz/Bilgi/Dikkat/Şüpheli/Tehlikeli, ATT&CK etiketli bulgular), paket yazılım tespiti; rapor "Özet" ve "Sistem" sekmeleri, bildirimde ilk 3 bulgu, sayfada karar rozeti, bildirim seviyesi ayarı, Oyun Modu'nda sessiz; Kaldırıcı'da "Kurulum izi var" rozeti ve rapordaki klasörlerin kalıntı adayı olması. Tek tık müdahaleler (5.2): açılış girdisi / IFEO değeri kaldır (.reg yedeği), hizmeti devre dışı bırak, görevi kapat, güvenlik duvarı kuralını kaldır, proxy kapat, kök sertifikayı kaldır (yedekli, geri alınabilir), Defender istisnasını kaldır. Kalan: karantina (5.3), ağ sensörü (2.6), USN/ETW (Faz 9) |
| Faz 8 | Tamam (PC testi bekliyor) | `main` | v3.23.0. Başlangıç (Event 100/101), TweakEngine + JSON, Hizmetler profilleri, Tepsi + Komut Paleti, Ayarlar + Sistem Bilgisi, Mağaza (winget), Olaylar zaman çizelgesi, Ağ İzleyici (Traceroute, DNS Benchmark, Metered), Avcı Modu koruması, S-17 TI dürüstlüğü |
| Faz 9 | Tamam (PC testi bekliyor) | `main` | Tam Koruma Modu (v4.0.0): NÖB Faz 8 (USN Journal, WMI KernelTrace, TamKorumaIpc), BakimShell asInvoker yürütücüsü ve IPC gelen kutusu, Windows Sandbox önizleme motoru (.wsb) ve diyalog entegrasyonları |
